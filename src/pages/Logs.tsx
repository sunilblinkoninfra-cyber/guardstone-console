...
import { coldLogs } from '@/data/mockData';
...
export default function Logs() {
  ...
  const filteredLogs = coldLogs.filter(
    (log) =>
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      ...
      <div className="bg-severity-cold/10 border border-severity-cold/20 rounded-lg p-4 text-sm">
        <p className="text-severity-cold font-medium">Cold emails only</p>
        <p className="text-muted-foreground mt-1">
          This log shows emails classified as low-risk (score &lt; 0.30) that were delivered without creating alerts.
        ...
        <table className="soc-table">
          ...
          <tbody>
            {filteredLogs.map((log) => (
              <tr key={log.id} className="hover:bg-muted/20 cursor-default">
                <td className="font-mono text-xs text-muted-foreground whitespace-nowrap">
                  <div>{format(new Date(log.timestamp), 'HH:mm:ss')}</div>
                  <div className="text-xs">{formatDistanceToNow(new Date(log.timestamp), { addSuffix: true })}</div>
                </td>
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>Showing {filteredLogs.length} of {coldLogs.length} entries</span>
